...
| 경우 | 무슨 일이 벌어지나 |
|---|---|
| **STC를 켰을 때** | 신호와 잡음을 같은 계수로 함께 줄이므로 SNR은 보존되지만 절대 레벨이 내려감. `stc_range_m` 안쪽에서는 잡음 바닥이 0 dB 아래로 내려가 화면 dB ≠ SNR (3.2절, 부록 G.3) |
...
**STC — 감도 시간 제어** (`enable_stc`, 같은 함수 안에서 압축 직후 적용)
...
| 거리 | 감쇠 (실측) | 이론 `40·log₁₀(R/6km)` |
|---|---|---|
| 1 km | −31.4 dB | −31.1 dB |
| 2 km | −19.2 dB | −19.1 dB |
| 3 km | −12.0 dB | −12.0 dB |
| 6 km 이상 | 0 dB | 0 dB |

> **대가** — 신호와 잡음을 **같은 계수로** 줄이므로 SNR은 보존되지만 절대 레벨이
> 내려감. 즉 `stc_range_m` 안쪽에서는 2.4절의 "화면 dB = SNR" 불변식이 깨짐.
> 기본값이 **꺼짐**인 이유이고, 이 콘솔이 계측기로 쓰이기 때문임. 실제 장비에서는
> 수신기 동적범위 보호가 더 중요하므로 보통 켜 둠.
...
```
CA:  α = N · (Pfa^(−1/N) − 1)
OS:  Pfa = Π_{i=0}^{k−1} (N−i)/(N−i+α)   → 이분법 60회로 정확해
TM:  절사 후 남은 개수로 CA 식
GO:  반쪽 창에 Pfa/2 로 CA 식 (표준 근사)
SO:  반쪽 창에 Pfa 로 CA 식 (낙관적)
```
...
`pwr_alpha_os`를 새로 풂(`pwr_cfar.c · pwr_cfar_run`의 OS 분기). 이분법 60회 반복에 각 회마다 k회
...
| `amplitude_db` · `snr_db` · `threshold_db` | 세기, SNR, 그 자리의 CFAR 임계값 |
...
stateDiagram-v2
    ...
    TENTATIVE --> FREE: 【시간】 마지막 측정 후 1.5 스캔<br/>(3.75 s) ← 오경보를 실제로 죽이는 규칙
    TENTATIVE --> FREE: 【장부】 연속 미스 2회 &amp; 총 히트 2회 미만
    TENTATIVE --> FREE: 【장부】 연속 미스 6회

    CONFIRMED --> COASTING: 【시간】 마지막 측정 후 1.0 스캔 (2.5 s)
    CONFIRMED --> COASTING: 【장부】 연속 미스 2회
    COASTING --> CONFIRMED: 다시 연관됨
    COASTING --> FREE: 【장부】 연속 미스 6회 (15 s)
    COASTING --> FREE: 【시간】 8 스캔 (20 s) 백스톱

    TENTATIVE --> FREE: 【가드】 발산 (속도 2배 초과 또는 P ≥ 1e12)
    CONFIRMED --> FREE: 【가드】 발산
    FREE --> [*]
```
...
| 상태 | 의미 | PPI 표시 |
|---|---|---|
| TENTATIVE | 개시됨, 아직 미확인 | 황색 **점만** (라벨·리더·트레일 없음) |
| CONFIRMED | M-of-N 충족 | 초록 심볼 + 번호 + 방향 벡터 |
| COASTING | 놓치는 중, 예측으로 유지 | 주황 |
| FREE | 삭제 | 없음 (경로 끝에 × 표시) |
...
| `enable_stc` / `stc_range_m` | 0 / 6 km | 100 m–1000 km | R⁻⁴ 보상 램프 (3.2절, 부록 F-4) |
...
| `land_clutter_rcs_dbsm` | −20 dBsm | 셀당 평균 σ⁰ |
...
**F-4. STC를 켰더니 근거리 잡음 바닥이 0 dB 아래로 내려감**

정상임. STC는 신호와 잡음을 **같은 계수로 함께** 줄이므로 SNR은 보존되지만 절대
레벨이 내려감. 감쇠량은 `40·log₁₀(R/stc_range)` dB이고 `stc_range` 밖은 1배.
실측(기본 `stc_range = 6 km`): 1 km에서 −31.1 dB, 2 km에서 −19.1 dB, 3 km에서
−12.0 dB, 6 km 이후 변화 없음. **그 구간에서는 "화면 dB = SNR"이 성립하지 않음.**
...
시나리오 없음(잡음만), 정지응시, `stc_range_m = 6 km`. A-scope dB.

| 거리 | STC 끔 | STC 켬 | 차이 | 이론값 `40·log₁₀(R/6km)` |
|---|---|---|---|---|
| 1 km | +9.0 | −22.4 | −31.4 | −31.1 |
| 2 km | +5.9 | −13.3 | −19.2 | −19.1 |
| 3 km | +6.5 | −5.5 | −12.0 | −12.0 |
| 6 km | +5.4 | +5.3 | 0.0 | 0 |
| 12 km | +6.6 | +6.6 | 0.0 | 0 |
...
### G.6 시나리오 4 — 정지 페어는 검열 노치에 들어감

4 스캔 후 방위 90°(정지 페어) 확정 트랙 수.

| `censor_zero_doppler` | `peak_selection` | 결과 |
|---|---|---|
| 1 (기본) | 1 (기본) | **트랙 없음** |
| 1 | 0 | 트랙 1개 (R = 12167 m) |
| 0 | 1 | 트랙 1개 |
...
검열을 끄면 A-scope에 12.16~12.25 km 구간에 1.4 dB 깊이의 이중 봉우리가 나타나지만,
두 표적 간격 35 m가 `init_inhibit_m = 600 m` 안이므로 **트랙은 어떤 설정에서도
하나**임. 방위 270°의 도플러 분리 페어는 네 경우 모두 두 트랙으로 정확히 유지됨.
