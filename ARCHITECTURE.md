...
경계를 넘는 구조체 배치는 `PWR_ABI_VERSION`(현재 7)에 종속되며, UI가 시작 시
...
| 단계 | 내용 |
|---|---|
| 기하 | ENU 좌표 → **유효지구** 보정 후 경사거리, 방위, 고각, 시선속도, 수평선 (`pwr_geom_compute`) |
| 안테나 | 2-way 가우시안 주엽. 설정한 부엽 레벨에서 `1/θ²` 포락선으로 인계. 고도축은 틸트와 csc² 채움 선택 가능 |
| 요동 | Swerling 모델별 진폭 배율 |
| 확장 | 선체 축을 따라 늘어선 산란점 열, 각자 자기 지연과 자기 위상 |
| 다중경로 | 저고각에서 해면 반사와의 간섭 (로빙). 왕복 경로이므로 진폭에 `F²` |
| 그림자 | 수평선 너머는 경험적 회절 감쇠 |
| 링크예산 | 레이다 방정식으로 SNR 산출 |
| 방위 컷 | **펄스마다** 재평가 (아래) |

**유효지구.** 모든 고도에서 지구 부풀음을 빼면 광선이 직선이 되고 표면이 평평해짐:

```
h'(d) = h − d² / (2 · k · a_earth)          k = env.refraction_k (기본 4/3)
```

이것이 평면지구 등가 변환이고, 하류의 모든 단계가 이미 가정하고 있는 형태로
문제를 바꿔 줌. 기본 24 km 표시 끝에서 부풀음이 34 m라, 이 보정 없이는 표적의
고각도 다중경로 경로차도 틀림. 보정만으로는 수평선 아래 표적이 가려지지 않으므로
평활구 수평선 `√(2·a_e·h_r) + √(2·a_e·h_t)`도 함께 계산해 그 너머에 경험적
그림자를 걸음 — 없으면 지상 레이다가 200 km 밖 해면 밀착 표적을 봄.

**CPI 안에서 빔이 움직임.** 안테나는 코히런트 묶음 동안 가만히 있지 않음. 기본
형상에서 CPI 한 번에 1.54°를 도는데 빔폭이 1.6°임. 패턴을 묶음 내내 얼려 두면
**주사 변조와 방위 빔 스플리팅이 둘 다 사라짐** — 둘 다 빔 형상 진폭 변조로
만들어지기 때문. 그래서 방위 컷은 펄스마다 다시 평가하고, `beam_azimuth_deg`는
그 소인의 **중심**으로 정의됨(플롯에 찍히는 보어사이트가 그것을 만든 조명의
평균이 되어 중심화에 편향이 생기지 않음). 재머도 연속파이므로 같은 방식으로
펄스마다 패턴을 통과함.
...
**클러터 필드의 구성 요소** (전부 `PWR_SimEnvironment`에서 제어)

| 요소 | 무엇 | 없으면 |
|---|---|---|
| 스페클 | 복소 가우시안 + AR(1) 상관 | — |
| 스펙트럼 폭 | 내부 운동 ⊕ **주사 변조** `0.265/t_dwell` | MTI 개선도가 회전 속도와 무관해짐 |
| 텍스처 | 단위평균 감마 변조 (`sea_shape_nu`) → K분포 | 검출기와 시뮬레이터가 같은 분포 가정을 공유 |
| 평균 도플러 | `clutter_mean_doppler_hz` | DC 노치가 공짜 승리가 됨 |
| 육상 섹터 | 별도 레벨·거의 정지 스펙트럼·해안선 | 연안 레이다인데 방위 의존성이 0 |

**주사 변조가 왜 스펙트럼 폭에 들어가는가.** 지연선 캔슬러가 지울 수 있는 것은
펄스간 상관이 유지되는 성분뿐이고, 그 상관을 깨뜨리는 것은 바다의 움직임만이
아님 — **빔이 패치 위를 지나가는 것 자체**가 진폭 변조이고, 그 변조가 스펙트럼을
넓힘. 둘은 제곱합으로 더해짐:

```
sigma_total² = sigma_internal² + (0.265 / t_dwell)²
t_dwell = 방위빔폭 / (6 · rpm)
```

기본 형상에서 내부 12 Hz에 주사 23.9 Hz가 붙어 26.7 Hz가 되고, 2펄스 개선도가
약 7 dB 줄어듦. 이 항이 없으면 **회전 속도를 바꿔도 MTI 성능이 변하지 않음** —
낙관적인 정도가 아니라 종류가 틀린 것임(T17).

**텍스처가 왜 필요한가.** 이 라이브러리의 CFAR 승수는 전부 셀 전력이 지수분포라는
가정에서 유도됨. 시뮬레이터의 바다도 복소 가우시안이면 검출기와 시뮬레이터가 같은
가정을 공유하므로, 측정한 Pfa가 설계값과 맞는 것은 **아무것도 증명하지 못함**.
복합 K 모델은 그 스페클에 느리게 변하는 감마 텍스처를 곱함 — 스케일 분리가
요점이라, 스페클은 밀리초로 탈상관하고 텍스처는 초 단위이므로 스캔당 한 번만
새로 뽑음. 그래야 스파이크가 CFAR를 속일 만큼 오래 남음. 거리 방향으로는 8셀씩
같은 값을 유지하는데, 실제 해면 텍스처가 한 분해능셀이 아니라 너울 규모의
상관길이를 갖기 때문(셀마다 독립으로 뽑으면 참조창이 여러 텍스처를 평균하는
최악 조건이 되어 실제보다 나쁘게 나옴).

**육상 섹터의 근사 하나.** 셀당 AR(1) 상태가 하나뿐이라 한 CPI가 두 스펙트럼을
동시에 나를 수 없음. 그래서 해안 방위에서는 육상 폭을 그 CPI 전체에 씀 — 해안이
가깝고 육상이 그 방위를 지배하는 경우에 맞는 근사이고, 그것이 이 기능이 존재하는
경우임.
...
| T15 | 유효지구 수평선 | 평면지구 — 수평선 너머 표적을 온전히 봄 |
| T16 | csc² 고도 채움 | 고정 고도 표적의 수신전력이 거리에 따라 변함 |
| T17 | 주사 변조가 MTI에 미치는 한계 | 개선도가 회전 속도와 무관해짐 |
| T18 | 스파이키 해면이 CFAR를 깨뜨림 | 시뮬레이터와 검출기가 같은 분포 가정을 공유 |
| T19 | 확장표적의 거리 확장 | 모든 표적이 1셀 — 확장표적 처리 기구가 자극받지 못함 |
| T20 | 선회 시 CV 필터 오차 | 시뮬레이터에 기동이 없어 추적기가 틀릴 수 없음 |

**T19·T20은 표적 자체를 검사함.** 표적이 등속 점산란체 하나뿐이던 동안에는
클러스터 허용치·최대셀 거부·드웰 병합 게이트가 전부 **1셀 자극**으로만 튜닝됐고,
CV 칼만 필터는 틀릴 수 있는 질문을 받은 적이 없었음. 확장표적은 산란점마다 자기
지연과 자기 반송파 위상으로 주입되므로 거리 확장·종횡비 의존 요동·글린트가 기하에서
저절로 따라 나옴. T20의 직진 168 m 대 선회 474 m — **이 2.8배가 IMM이 메우려는
간격**이고, 만들기 전에 재어 두는 것이 요점임.

**스위트는 두 갈래로 실행됨.** 케이스는 비용으로 깨끗하게 갈림 — 대부분은 대수적
검사나 단일 CPI 검사라 밀리초로 끝나고, 다섯 개는 수천 CPI를 적분해야 무언가를
말할 수 있음. 후자를 돌리는 것은 게이트에 옳고 빌드 스크립트에 그름: 몇 분간
침묵하는 스크립트는 멈춘 것과 구별되지 않고, 운용자가 그 출력을 읽지 않게 됨.

| 진입점 | 무엇 | 언제 |
|---|---|---|
| `pwr_self_test_quick` | 적분이 필요 없는 전부 (약 1초) | `build.bat` |
| `pwr_self_test` | 위 + 회전 드웰·회전 트랙·Pd·K 클러터·기동 (약 20초) | CTest, CI |

어느 쪽에도 속하지 않는 케이스는 없음.

**콘솔은 시작 시 검증을 첫 프레임 이후로 미룸.** `app_create` 안에서 돌리면 창은
존재하지만 한 픽셀도 그려지지 않은 채 스위트가 끝나기를 기다리게 되고, 그려지지
않은 창은 그냥 검은 사각형이라 멈춘 것과 구별할 수 없음. 게다가 사용자가 기다리고
있는 경로에 통계 케이스를 넣을 이유가 없으므로, 지연 실행되는 것은 quick 집합임.

**T15–T18은 전파·클러터 물리를 바깥 기준에 맞춰 검사함.** T13이 검출 통계를
비중심 카이제곱과 맞추듯, 이 넷은 각각 평활구 수평선 공식, csc² 안테나의
정의적 성질, 0.265/t_dwell 주사 변조, 그리고 K분포의 형상모수 의존성을 기준으로
씀. 특히 T18 이전에는 시뮬레이터의 바다가 검출기와 **같은 지수분포 가정**을
공유하고 있어서, 측정한 오경보율이 설계값과 맞는 것이 아무것도 증명하지 못했음.
...
T17 scan modulation limits MTI  29.2 dB at 24 rpm, 35.1 dB at 6 rpm (+5.9 dB)
T18 spiky sea defeats a Rayleigh CFAR  Rayleigh 3906, nu=2 5442, 0.6 10217, 0.2 18782
18/18 cases passed
```
...
| `elevation_tilt_deg` | 0° | −30–60 | 고도축 보어사이트 |
| `elevation_csc2_deg` | 0 (끔) | 0–85 | csc² 채움 상단. 켜면 고정 고도 표적의 수신전력이 거리에 무관해짐 |
| `sidelobe_level_db` | −45 dB | −80–−10 | 부엽 포락선 기준. 이 레벨에서 가우시안이 `1/θ²`로 인계됨 |
| `refraction_k` | 4/3 | 0.5–6 | 유효지구 계수. 표적 고도 보정과 수평선을 함께 정함 |
| `sea_shape_nu` | 0 (레일리) | ≥0 | K분포 형상. 작을수록 스파이키. 0.1–1이 거친 바다, 10 이상은 레일리와 구분 안 됨 |
| `clutter_mean_doppler_hz` | 0 | — | 해면 벌크 드리프트. 0이면 DC 노치가 공짜 승리가 됨 |
| `land_bearing_deg` / `land_width_deg` | 0 / 0 (육상 없음) | — | 육상 섹터. 폭이 0이면 열린 바다 |
| `land_clutter_to_noise_db` / `land_spread_hz` / `land_range_min_m` | 45 dB / 1 Hz / 3 km | — | 육상 레벨, (거의 정지한) 스펙트럼, 해안선 거리 |

**표적 정의** (`PWR_SimTarget`, 시나리오가 채움)

| 필드 | 기본 | 의미 |
|---|---|---|
| `accel_mps2` | 0 | 침로 방향 가속. 속력만 바꾸고 침로는 유지 |
| `turn_rate_dps` | 0 | 선회율, + 가 우현. 속력은 유지하고 침로만 바꿈 |
| `length_m` / `scatterers` | 0 / 0 | 선체 길이와 산란점 개수. 둘 다 유효할 때만 확장표적이 되고, 아니면 종래의 점산란체 |

속도는 `PWR_SimTarget`이 아니라 시뮬레이터 상태에서 적분됨 — 가속과 선회가 있으면
속도는 정의가 아니라 상태이기 때문. 표적 구조체는 초기 조건을 갖고, 상태가 현재
값을 가짐. 발행되는 진실값에는 **현재** 속도가 실림.
