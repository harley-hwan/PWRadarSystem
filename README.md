...
M-of-N 히트/미스는 **스캔(안테나 1회전)당 1회** 적립됩니다. 드웰 병합된
플롯(§2.7)은 빔이 표적을 지나간 뒤에야 도착하므로 조사 CPI마다 판정할 수
없고, 대신 조준선이 트랙 방위를 LAG(빔폭의 1.6배 + 2 CPI 여유)만큼
지나치는 순간 — 그 드웰의 플롯이 이미 발행·연관된 시점 — 에 한 번
판정합니다. 예측이 뒤처져 교차가 플롯보다 먼저 와 미스가 기록됐다면 이후의
연관이 장부를 소급 정정하고, 역행 겉보기 운동으로 교차 창을 건너뛴 트랙은
시간 기반 캐치업(1.25 스캔)이 회전당 1회 판정을 보장합니다. 연관 후보는
고정 방위 창이 아니라 **비용 행렬과 동일한 직사각 미터 게이트**
(`gate_max_range_m`)로 선정합니다 — 방위 창은 통계 게이트보다 좁아, 접선
150 m/s 횡단 표적처럼 스캔당 10°를 움직여도 예측에서 375 m밖에 안 떨어진
플롯을 자기 트랙에서 끊어버리기 때문입니다. 여기에 **스캔 기반 노후화
백스톱**이 남아 있습니다 — 커버리지 밖으로 나가 빔이 더 이상 지나지 않는
트랙을 회수하기 위한 것으로, 시한은 `delete_misses + 2` 스캔이라 운용자의
삭제 설정을 선점하지 않습니다. 정지응시(스캔 0 rpm)에서는 예전 그대로
CPI당 1회 판정합니다. 도플러 게이트는 측정이 ±V_ua로 접혀 들어오는 것을
감안해 **2·V_ua 모듈로**로 비교합니다 — 접힘을 무시하면 실제 시선속도가
모호 경계를 넘는 순간(예: 횡단 표적의 방위가 커질 때) 수렴한 트랙이 모든
후속 플롯을 기각하고 굶어 죽습니다. 시나리오 6이 검증하는 항목입니다.
...
### 2.7 플롯은 CPI가 아니라 드웰 단위입니다

CPI별 탐지의 방위각은 그 CPI의 빔 조준선이고, 강한 표적은 빔이 지나가는
여러 연속 CPI에서 전부 임계값을 넘습니다(SNR 66 dB 표적은 2-way −50 dB
지점까지 탐지됩니다). 병합 없이 흘리면 표적 하나가 스캔마다 방위 수 도(°)에
걸친 **플롯 문자열**을 만들고, 그 결과 (1) 남는 가장자리 플롯이 개시 억제
반경을 벗어나 **중복 트랙**을 낳고, (2) CPI당 1.5°씩 스텝하는 방위 갱신이
칼만 필터에 빔 회전 방향의 **가짜 접선 속도**를 주입해 실속도와 무관한 —
심지어 반대 방향의 — 벡터가 나옵니다.

그래서 실장비 플롯 추출기처럼 **드웰 단위로 중심추정**합니다: 연속 CPI에서
거리·시선속도가 한 셀 이내로 일치하는 히트를 하나의 드웰 플롯에 전력 가중
누적하고, 빔이 지나가면(기여 없는 CPI 1회) 닫아서 발행합니다. 방위는 2-way
빔 형상에 대한 전력 가중 중심 — 고전적인 빔 분할(beam splitting) 추정기 —
이라 빔폭의 수분의 일 정확도가 나옵니다. 거리/속도 게이트는 의도적으로
한 셀 수준으로 좁아서 분해능 페어(시나리오 4)는 병합되지 않고, 같은 CPI의
플롯끼리는 정의상 서로 다른 표적이므로 병합하지 않습니다. 정지응시에서는
드웰이 끝나지 않으므로 이 단계를 통째로 우회합니다.
...
드웰→히트→플롯→트랙으로 이어지는 처리 사슬을 화면에서 직접 검증할 수
있습니다. PPI에는 (1) 보존 시간 동안 남는 **플롯 히스토리**(청록 = 트랙에
연관된 플롯, 황색 = 미연관 — 오경보 산포가 그대로 보입니다), (2) 트랙·실제값의
**전체 경로 폴리라인**(나이에 따라 감쇠, 확정 트랙 소실 지점은 ×), (3) 이번
드웰에 어떤 플롯이 어떤 트랙을 갱신했는지 잇는 **연관선**, (4) 트랙 심볼 위의
**드웰 히트/미스 링**(초록 = 히트, 빨강 점선 = 미스)이 겹쳐집니다. 트랙
테이블의 HIT 열과 선택 트랙 판독의 dwell window는 M-of-N 확인 논리가 세는
비트 증거를 그대로 보여줍니다. 우측 **Verify 탭**은 실제값-트랙 자동 페어링으로
표적별 추적 상태·현재 오차·RMSE·추적 완성도(CMP%)·최초 트랙 소요시간(TTT)과
전역 요약(추적률·가짜 트랙·중복 트랙)을 집계합니다. 보존 시간과 레이어는
View 탭에서 조절합니다.

트랙의 속도는 **화살촉이 달린 이동 방향 벡터**(MIL-STD-2525 스타일)로
표시됩니다. 길이는 View 탭의 leader 시간(기본 60 s) 동안의 이동 거리이고,
30초 이동량마다 눈금이 찍히므로 거리 링에 대고 속도를 정량적으로 읽을 수
있습니다. `T` 오버레이를 켜면 실제 침로가 점선 화살표로 겹쳐져, 추정 침로
오차가 두 화살촉 사이의 각도로 바로 보입니다.
...
* 그 외 모든 `pwr_engine_*` 세터 — 어느 스레드에서든 안전하고 **논블로킹**.
  변경은 펜딩 메일박스에 적재되어 워커가 다음 CPI 시작 시 반영하고, 엔진이
  유휴 상태(정지·일시정지·수동 스텝)면 그 자리에서 즉시 적용됩니다. 호출
  스레드는 어떤 경우에도 진행 중인 CPI가 끝나기를 기다리지 않습니다 —
  워커가 부하율 1을 넘겨 `proc_lock`을 사실상 상시 점유하는 상황에서도
  UI의 슬라이더 드래그가 CPI 시간만큼 멈추지 않게 하기 위한 설계입니다.
  예외적으로 `pwr_engine_reconfigure`와 표적 목록 변경자는 현재 CPI가
  끝나기를 기다립니다(락 대기 최대 1 CPI + 재구성 수 ms). 이 상한은 공정성
  게이트가 보장합니다 — 일반 뮤텍스는 대기자에게 소유권을 넘기지 않아
  포화된 워커가 락을 즉시 재획득하며 뮤테이터를 수 초씩 굶길 수 있으므로,
  뮤테이터가 대기를 선언하면 워커는 CPI 사이에서 양보합니다.

락 순서에 의한 교착은 불가능합니다: `ctrl_lock`은 실행 상태와 조건변수를,
`proc_lock`은 1 CPI 처리 전체 구간과 재구성을 감싸고, `pending_lock`은
메일박스 복사 구간에만 잡는 리프 락입니다. 워커는 `ctrl → 해제 → proc →
pending`, 변경자는 `proc → 해제 → ctrl` 또는 `pending` 단독으로만 잡으므로
중첩 방향이 `proc → pending` 하나뿐입니다.

버퍼 크기가 바뀌는 재구성에서도 **트랙 파일은 보존됩니다** — 트래커 상태는
격자 인덱스가 아니라 ENU 미터 좌표라 격자 변경으로 무효화되지 않습니다.
새 커버리지 밖에 남은 트랙은 플롯을 더 받지 못해 스캔 노후화 규칙으로
자연 소거됩니다.
