...
![PWRadarUI 전체 화면](docs/ui-overview.png)

각 항목의 의미와 효과는 [§4 콘솔 화면 구성](#4-콘솔-화면-구성)에 항목별로 정리돼 있음.
...
아래는 실제 실행 화면임(시나리오 3 "Mixed air and surface picture", 9.4초 경과).
`--capture`로 뽑은 것이라 문서와 코드가 어긋날 수 없음.
...
| 구역 | 위치 | 역할 |
|---|---|---|
| ① 툴바 | 최상단 | 실행 제어 · 시나리오 · 속도 · 부하 판독 |
| ② 제어 패널 | 좌측 (6탭) | 모든 설정 노브 + 파생값 판독 |
| ③ 디스플레이 | 중앙 (4분할) | PPI · R-D 맵 · A-scope · RTI |
| ④ 보고 패널 | 우측 (5탭) | 트랙·플롯 표, 채점, 표적 목록, 로그 |
| ⑤ 상태바 | 최하단 | 격자·타이밍 요약 |

모든 경계는 드래그 가능한 스플리터임.
...
**PPI (좌상)** — 운용자가 실제로 보는 그림.

| 요소 | 뜻 |
|---|---|
| 초록 회전 광선 | 현재 빔 조준선. 뒤로 감쇠하는 쐐기 14조각이 잔광 |
| 거리 링 + 라벨 | 1-2-5 알고리즘으로 자동 배치 (5/10/15/20 km) |
| 중앙 검은 원 | 블라인드 거리 3 km — 송신 중이라 못 봄 |
| `T001` 초록 원 | 확정 트랙 + 번호. 심볼 모양은 **추정 속력**에서 나옴(원=항공, 사각=수상, 삼각=회전익/UAV, 다이아=미사일) |
| 화살표 | 이동 방향 벡터. 길이 = leader 시간(기본 60 s) 동안의 이동거리, 30초마다 눈금 |
| 노란 점 | tentative 트랙. 대부분 오경보이므로 라벨·리더 없이 조용히 그림 |
| 작은 점 산포 | 플롯 히스토리. 청록 = 트랙에 연관됨, 황색 = 미연관 |
| 우상단 `AZ 268.61°` | 현재 조준선 방위 |

**Range-Doppler 맵 (우상)** — **한 CPI의 원본 데이터.** 가로 = 거리, 세로 = 시선속도.

가로줄처럼 보이는 밝은 띠가 표적이고, 세로 위치가 곧 속도임. 세로 0 근처의
넓은 얼룩은 해면 클러터(거의 정지). 컬러바 눈금은 dB이며 §3.1에 따라 **곧 SNR**임.
좌우 끝이 접히는 이유는 도플러 스펙트럼이 PRF 주기로 반복되기 때문(§1.5).

**A-scope (좌하)** — 가로 = 거리, 세로 = dB. 세 트레이스가 겹쳐 그려짐.

| 트레이스 | 뜻 |
|---|---|
| 회색 `raw video` | 압축 직후, **MTI·클러터 주입 이전** (부록 F-6) |
| 파란 `max over Doppler` | 거리마다 도플러축 최댓값. 실제 탐지가 보는 값 |
| 붉은 `CFAR threshold` | 그 셀의 적응 임계값 |

**파란 선이 붉은 선을 넘는 곳이 탐지**임. "왜 안 잡히지?"의 답이 여기 있음.
근거리에서 두 선이 함께 솟은 것이 해면 클러터이고, CFAR가 임계를 같이 올려
오경보를 막고 있는 모습임.

**RTI 워터폴 (우하)** — 가로 = 거리, 세로 = 경과 시간(아래로 갈수록 과거).

수평 줄무늬 하나가 표적 하나의 궤적이고, **기울기가 거리 변화율**임. 수직으로
길게 뻗은 띠가 클러터. 탭을 바꾸면 `Doppler spectrum`(커서 거리빈의 속도 단면)이
나옴.

> RTI만 줌·팬이 유지되지 않음(부록 F-9).

### 4.3 ② 제어 패널 6탭

![제어 패널 6탭](docs/ui-control-tabs.png)

왼쪽부터 `RF` `DSP` `CFAR` `Track` `Scene` `View` (`1`–`6` 키).

**RF — 송신기·안테나·링크 예산**

| 항목 | 효과 |
|---|---|
| carrier / bandwidth | 파장과 **거리 분해능**을 정함. 대역폭은 탐지거리를 바꾸지 않음(§7.5) |
| pulse / sample / PRF | 에너지·블라인드 거리 / 거리 빈 간격 / **비모호 거리↔속도**(§7.2) |
| peak power | 링크 예산. 2배 올려도 탐지거리는 +19 % |
| az BW / scan / gain | 방위 분해능 · 갱신률 · 링크 예산 |
| losses / noise fig | 시스템 손실과 잡음지수 |
| **DERIVED** 블록 | 위 값에서 유도된 결과 — 파장, 분해능, 압축이득, 비모호 거리/속도, 듀티비, 잡음전력 |
| **DETECTION RANGE** | 13 dB SNR 기준 RCS별 탐지거리. 노브를 돌리면 즉시 갱신되므로 **링크 예산 계산기로 쓸 수 있음** |

**DSP — 코히런트 처리**

| 항목 | 효과 |
|---|---|
| pulses / CPI | 속도 분해능 ↑, CPI 길어짐. **빔폭당 펄스 수(33)를 넘기지 말 것**(§7.4) |
| Doppler FFT | 오버샘플 배수. 분해능은 안 좋아지고 봉우리 위치만 매끄러워짐 |
| range taper | 거리 부엽 ↔ 주엽폭 상충(§3.2) |
| Doppler taper | 도플러 부엽. 정지 표적의 부엽 플롯 문제와 직결(§3.9) |
| MTI | 고정물 제거. 차수만큼 펄스를 소모함 |
| pulse compression / Doppler processing | 끄면 각 단계의 기여를 분리해 볼 수 있음 |
| sensitivity time control | 근거리 R⁻⁴ 보상. **켜면 그 구간의 dB가 SNR이 아니게 됨**(부록 F-4) |
| span / range bins | 표시 범위와 격자. 좁히면 고속시간 FFT가 작아져 부하가 줄어듦 |
| **STAGE TIMING** | 단계별 처리시간(EWMA)과 `total / budget`. 어느 단계가 비싼지 한눈에 보임 |

**CFAR — 검출기와 플롯 추출**

| 항목 | 효과 |
|---|---|
| estimator | CA/GO/SO/OS/TM. **OS·TM은 거리축 1-D 창**이라 도플러 노브가 무력해짐(§3.7) |
| Pfa | 셀당 오경보 확률. 로그 슬라이더 |
| bias | 계산된 임계에 얹는 운용자 보정 |
| guard R/D · train R/D | 참조창 기하. **train R / guard R 변경은 전체 재구성**을 유발 |
| OS rank | OS의 k. 0이면 0.75·N |
| censor zero Doppler | DC 열 검열. 끄면 정지 표적이 보이기 시작함(부록 F-1) |
| peak selection | 가드 영역 내 최댓값만 채택. 끄면 십자 모양 플롯 무리가 생김 |
| cluster / tol R / tol D / min SNR | 셀을 플롯으로 묶는 규칙 |
| **MEASURED** | 실측 잡음 바닥, **달성 Pfa**, 설계 플롯/스캔, 검사 셀 수. 설계 대비 실측을 여기서 바로 비교 |

**Track — 추적기**

| 항목 | 효과 |
|---|---|
| association | 탐욕 vs 전역 최적(JV) |
| σ accel | **가장 중요한 노브.** 기동 추종 ↔ 부드러움 |
| σ range / az | 측정 불확실성. 게이트 크기를 정함 |
| gate σ / gate cap | 마할라노비스 게이트와 직사각 상한 |
| confirm M / of N | 확정 규칙. 판정은 **회전당 1회** |
| coast after / delete after | 코스팅·삭제 문턱 |
| init inhibit | 이 반경 안에는 새 트랙을 만들지 않음 → 중복 트랙 억제 |
| use Doppler in the gate | 2·V_ua 모듈로 비교. 끄면 시나리오 6이 깨짐 |
| **TRACK FILE** | 활성/확정/생성/삭제 누적 |

**Scene — 환경과 표적 주입**

| 항목 | 효과 |
|---|---|
| thermal noise | dB 기준 그 자체. 끄면 잡음 바닥이 사라짐 |
| sea clutter / sea state / CNR / clutter BW | 해면 반사 세기와 도플러 폭 |
| rain | 강우 클러터 (R⁻² 법칙) |
| noise jammer / jammer az / JNR | 특정 방위의 잡음을 들어 올림 → PPI 재밍 스트로브 |
| surface multipath | 저고각 로빙 |
| transmit eclipsing | 블라인드 거리 생성 |
| range ambiguity fold | 켜야 거리 모호가 관찰됨(기본 꺼짐) |
| **INJECT A TARGET** | 거리·방위·고도·침로·속력·RCS·요동 모델을 지정해 즉석 주입 |

**View — 표시 전용 (물리에 영향 없음)**

| 항목 | 효과 |
|---|---|
| colormap / video gain / persistence | PPI 색·밝기·잔광. **persistence 0은 "감쇠 없음"**(부록 F-3) |
| scale / leader | 거리 스케일, 방향 벡터 길이의 시간 기준 |
| 레이어 체크박스 | 비디오·링·빔·플롯·트랙·트레일·공분산 타원·실제값·라벨 |
| HISTORY / VERIFICATION | 경로·플롯 히스토리·연관선·히트/미스 링, 보존 시간 |
| RANGE-DOPPLER MAP | 컬러맵과 색상 한계(자동/수동) |
| A-SCOPE / WATERFALL | raw 트레이스·임계 트레이스 표시 여부, RTI 컬러맵 |
