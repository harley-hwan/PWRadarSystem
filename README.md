...
> 그림이 안 보이는 환경에서도 내용이 빠지지 않도록, **각 그림 아래에 같은 내용을
> 표로도 실어 둠.** 그림은 위치를 짚어 주는 보조 자료이고, 표만 읽어도 충분함.
> (GitHub 웹에서 저장소 이미지는 `raw.githubusercontent.com`에서 오므로, 그 호스트가
> 막힌 망에서는 그림이 뜨지 않음. 로컬 클론 + 편집기 미리보기로는 정상적으로 보임.)
...
| 화면 요소 | 무엇인가 |
|---|---|
| 초록 회전 광선 | 현재 빔 조준선. 뒤로 갈수록 어두워지는 쐐기 14조각이 잔광. 2.5초에 한 바퀴 |
| 거리 링 (5/10/15/20 km) | 1-2-5 알고리즘으로 자동 배치. View 탭 `scale`로 범위 변경 |
| 중앙 검은 원 | **블라인드 거리 3.0 km** — 송신 중이라 못 봄. 펄스폭을 늘리면 커짐 |
| `T001` 등 심볼 + 번호 | 트랙. 색이 신뢰도(아래 표), 번호가 고유 ID |
| 심볼 **모양** | 추정 속력 분류 — ○ 항공 · □ 수상 · △ 회전익/UAV · ◇ 미사일. 진실 분류가 아니라 추적기가 잰 속력에서 나옴 |
| 화살표 (leader) | 이동 방향. 길이 = leader 시간(기본 60 s) 동안의 이동거리, 눈금 = 30초 이동량 |
| 작은 **청록** 점 | 플롯 히스토리 — 트랙에 연관된 관측 |
| 작은 **황색** 점 | 플롯 히스토리 — 미연관 관측. 오경보 산포가 그대로 보임 |
| 우상단 `AZ 268.61°` | 현재 조준선 방위 (나침반 방위, 0° = 북) |

조작: 휠 = 거리 스케일, 좌클릭 = 트랙 선택, 더블클릭 = 전체 스케일,
`T`/`G`/`H` = 실제값 / 공분산 타원 / 히스토리 오버레이.
...
| 화면 요소 | 무엇인가 |
|---|---|
| 가로로 번진 밝은 띠 | **해면 클러터.** 파도는 거의 안 움직이므로 시선속도 0 부근에 몰림. R⁻³으로 감쇠 |
| 가운데 점선 | **시선속도 0** = 도플러 행 31. 기본 설정은 이 행 ±1을 CFAR 검사에서 제외(§3.9) |
| 배경 파랑 | 잡음 바닥 **정확히 0 dB**. 세로 줄무늬는 거리 빈별 잡음 요동 |
| 점으로 뭉친 밝은 곳 | 표적. 세로 위치가 곧 시선속도 |
| 컬러바 | dB인데 곧 SNR. 잡음 바닥이 0 dB로 캘리브레이션돼 있으므로 |
| 세로축 범위 | ±73.7 m/s = 비모호 속도. 이보다 빠르면 접혀 들어옴 |
| 가로축 범위 | 0~24 km = 표시 범위. 비모호 거리 50 km 안쪽 |

판단 요령 — **가로로 길게 번진 것은 클러터, 점으로 뭉친 것은 표적.** 표적이 위아래
끝에 붙어 있으면 속도가 접혔을 가능성을 의심할 것.
...
| 트레이스 | 무엇인가 | 잡음만 있을 때 높이 |
|---|---|---|
| 회색 `raw video` | 압축 직후. **MTI 이전이자 클러터 이전** | **0 dB** |
| 파랑 `max over Doppler` | 거리마다 도플러축 최댓값. 실제 탐지가 보는 값 | **+6 dB** (64셀 최댓값이라 `10·log₁₀(ln 64)` 만큼 들림) |
| 빨강 `CFAR threshold` | 그 셀의 적응 임계값 | **+12 dB** (배경 × 임계배수 16.5) |

**(가) 잡음만** — 세 트레이스가 위 높이에 정확히 앉음. 이 문서가 계산으로 주장한
값이 화면으로 확인되는 지점임.

**(나) 해면 클러터** — 배경이 20 dB 넘게 솟자 **붉은 임계선이 같이 올라감.** 고정
임계였다면 3~10 km가 통째로 오경보가 되거나, 그걸 피하려 임계를 올리면 먼 거리
표적을 전부 놓쳤을 것임. 두 선의 간격이 클러터 안에서도 12 dB로 유지되는 것이
CFAR가 하는 일의 전부임.
...
| 탭 | 주요 노브 | 무엇을 바꾸는가 |
|---|---|---|
| **RF** | bandwidth | 거리 분해능 `c/2B` (5 MHz → 30 m). **탐지거리는 안 바뀜** |
| | PRF | 비모호 거리 ↔ 속도. 곱이 상수라 둘 중 하나만 살 수 있음 |
| | DERIVED / DETECTION RANGE | 파생값과 RCS별 탐지거리가 즉시 갱신 = **링크 예산 계산기** |
| **DSP** | pulses / CPI | 속도 분해능 ↑, CPI 길어짐. **빔폭당 33발이 상한** |
| | range taper | 거리 부엽 ↓ ↔ 주엽 넓어짐(분해능 ↓) |
| | MTI | 고정물 제거. 차수만큼 펄스를 소모함 |
| | STAGE TIMING | 단계별 ms와 `total/budget`. 1 초과면 실시간 미달 |
| **CFAR** | estimator | OS·TM은 거리축 1-D 창이라 도플러 노브가 무력해짐 |
| | Pfa | 10배 올려도 임계는 0.7 dB만 완화되고 오경보는 10배 |
| | censor zero Doppler | 끄면 정지 표적이 보이기 시작함 |
| | MEASURED | 설계 Pfa 대비 실측을 바로 비교 |
| **Track** | σ accel | **추적기의 가장 중요한 노브.** 기동 추종 ↔ 부드러움 |
| | confirm M / of N | 확정 속도 ↔ 가짜 트랙. 판정은 회전당 1회 |
| | init inhibit | 이 반경 안에는 새 트랙 없음 → 중복 트랙 억제 |
| **Scene** | sea state / CNR | 클러터 세기. 등급당 5 dB, 기준 거리는 1 km |
| | INJECT A TARGET | 거리·방위·속력·RCS를 지정해 즉석 주입 |
| **View** | persistence | PPI 잔광. **0은 "감쇠 없음"** — 화면이 탐 |
...
| 4 | Resolution pair (range + Doppler) | 거리 분해능, 도플러 분리, OS-CFAR 다표적 | 270° 쪽 두 트랙이 **분리 유지**되는지, 90° 쪽은 A-scope 봉우리 폭 (아래 주의) |
| 5 | Crossing tracks | 데이터 연관 (동일 셀 통과) | t ≈ 93 s 3중 교차 후 트랙 **ID가 바뀌지 않는지** |
...
| 2 | SHIP-01 | 20° | 6.5 km | 8 m | 110° / 7 m/s | 2000 m², 선체 150 m | 1 |
| 2 | SHIP-02 | 118° | 11.5 km | 6 m | 300° / 11 m/s | 800 m², 선체 110 m | 1 |
| 2 | BOAT-03 | 250° | 4.2 km | 3 m | 35° / 14 m/s | 25 m² | 3 |
| 2 | BUOY-04 | 305° | 8.8 km | 2 m | 정지 | 10 m² | 0 |
| 4 | PAIR-A / PAIR-B | 90° | 12 km / +1.2·ΔR | 2000 m | 90° / 60 m/s (동일) | 5 m² | 0 |
| 4 | DOPP-C / DOPP-D | 270° | 12 km (동일) | 2000 m | ±120 m/s (반대) | 5 m² | 0 |
| 5 | CROSS-A / CROSS-C | — | 약 18 km | 약 3000 m | 150 m/s, 서로 마주봄 | 8 m² | 1 |
| 5 | CROSS-B | 0° | 23.2 km | 3050 m | 정면 접근 120 m/s | 8 m² | 1 |
...
| 8 | WALK-01 / -02 | 0° / 180° | 2 km 출발 | 200 m | 외향 200 m/s | 1 m² / 10 m² | 1 |
...
> **시나리오 4 주의 — 분해능은 플롯 수준 항목이지 트랙 수준 항목이 아님.**
> 방위 90°의 거리 페어(PAIR-A/B)는 같은 침로·같은 속도로 함께 벌어지므로 간격이
> 1.2 분해능셀로 유지되고 도플러 빈도 공유함. 즉 순수한 **거리** 분해능 시험임.
> 속도가 0이 아닌 것이 중요한데, 정지 표적이면 §3.9의 제로도플러 검열 노치에
> 들어가 둘 다 탐지되지 않고 시나리오가 아무것도 검증하지 못하기 때문임.
>
> A-scope에서 두 표적은 약 70 m 폭의 봉우리 하나로 보임 — 24 m 빈에 36 m 간격이라
> 두 정점이 1.5 빈밖에 안 떨어져 있고, 테일러 −35 dB 테이퍼가 주엽을 더 넓힘.
> 분해능을 눈으로 확인하려면 RF 탭에서 대역폭을 올려 분해능셀을 좁힐 것.
>
> 두 표적은 35 m 떨어져 있어 `init_inhibit_m = 600 m` 안이므로 **트랙은 끝까지
> 하나만 생성됨.** 시나리오 4에서 확정 트랙이 3개(페어 1 + DOPP 2)로 나오는 것이
> 정상임.
>
> 방위 270°의 **도플러 분리 페어**(DOPP-C/D)는 t = 0에서 거리가 같고 시선속도만
> ∓118 m/s로 다른 두 표적이 서로 다른 확정 트랙으로 유지되는지를 봄. 같은 CPI의
> 플롯은 병합하지 않는다는 드웰 병합 규칙이 여기서 검증됨.

> **시나리오 5 주의 — CROSS-B의 속도는 임의 값이 아님.** 셋 중 CROSS-B만 정면
> 접근이라 속도 전체가 시선속도가 됨. 150 m/s로 두면 첫 블라인드 속도
> (λ·PRF/2 = 147.4 m/s)에서 3 m/s 이내라 제로도플러 열로 접혀 CFAR 검열에
> 삭제되고, 3중 교차가 2중 교차로 주저앉음. 120 m/s는 노치를 벗어나며, 출발
> 거리를 23.2 km로 맞춰 세 표적이 t ≈ 93 s에 (0, 12 km)에서 동시에 만나는 기하는
> 그대로 유지함.
...
T14 scenario serialisation      3792 bytes, 6 targets both sides, round trip exact, invalid file refused
T15 radar horizon               horizon 29.8 km; 20->25 km 4.7 dB (R^-4 = 3.9), 25->33 km 25.3 dB
T16 cosecant-squared coverage   constant-altitude spread: Gaussian 43.4 dB, csc2 0.27 dB
T17 scan modulation limits MTI  2-pulse clutter attenuation 29.2 dB at 24 rpm, 35.1 dB at 6 rpm
T18 spiky sea defeats a Rayleigh CFAR  false plots/3 scans: Rayleigh 3022, nu=2 3548, 0.6 6843, 0.2 9125
T19 extended target range extent  -10 dB echo extent: point 48 m, 150 m hull 240 m
T20 a turn does not cost the tracker its lock  straight 167 m / 78% held, 6 deg/s turn 157 m / 78% held
T21 velocity seed only when unambiguous  ambiguous radar 0.0 m/s, unambiguous 39.3 m/s
21/21 cases passed
```

**T20·T21은 추적기의 두 결함을 고정함.**

- **T20** — 원래 "선회가 직진보다 1.8배 이상 비싸야 한다"고 주장하는 테스트였음. 즉
  **결함의 존재를 검증**하고 있었음. 그 2.8배는 운동모델이 아니라 프로세스 잡음에서
  나왔음: 이산 백색가속도 Q를 CPI마다 적용하는데 측정은 회전당 한 번 들어오므로,
  2.5초 재방문을 234 CPI로 쪼개면 누적 속도 분산이 234분의 1이 됨. 운용자가 σ_a =
  6 m/s²를 넣으면 필터는 0.39로 동작하고, 배율은 `pulses_per_cpi`에 따라 조용히 변함.
  연속시간 형태로 바꾸니 선회 비용이 사라짐(474 → 157 m). 판정은 절대 미터와
  **완결도 하한**으로 바뀜 — 게이트가 닫혀 놓쳤다 재획득하는 현상은 RMSE로 숨지만
  (잡고 있는 구간은 정확하므로) 유지 시간 비율로는 드러나기 때문.
- **T21** — 도플러 축은 정확히 ±Vua를 덮으므로 모든 시선속도가 접혀서 도착하고, 단일
  플롯은 어느 접힘인지 알 수 없음. 그런데 개시 코드가 그 값을 속도로 시드하고
  있었음: 120 m/s로 접근하는 표적이 **+27 m/s**로 읽혀 필터가 멀어지는 표적으로
  출발함. 판단 기준은 측정값이 아님(접힌 값은 구간 어디에나 균일하게 떨어짐) —
  **이 레이다가 허용 표적을 접을 수 있는가**, 즉 `Vua >= max_speed_mps`임. 양쪽
  다리를 같은 기하에서 검사하므로 어느 쪽으로도 우연히 통과할 수 없음.

**전체 스위트는 게이트, 빠른 집합은 빌드용.** `--selftest`는 20개 전부를 돌리며
약 20초 걸림 — 네 개의 통계 케이스(회전 드웰·회전 트랙 수명주기·Pd·K 클러터·기동)가
수천 CPI를 적분해야 하기 때문. 빌드 스크립트가 기다리기에는 긴 시간이고, 몇 분간
아무 출력이 없는 스크립트는 멈춘 것과 구별되지 않으므로:

```sh
./build/PWRadarUI --selftest quick   # 적분이 필요 없는 전부, 약 1초
./build/PWRadarUI --selftest         # 게이트 전체 (CTest가 도는 것)
```

`build.bat`은 빠른 집합을 돌림. 두 집합 어디에도 속하지 않는 케이스는 없음.

**콘솔의 시작 시 자체검증은 첫 프레임이 그려진 뒤에 돌아감.** `app_create` 안에서
돌리면 창은 만들어졌지만 한 번도 그려지지 않은 채로 스위트가 끝날 때까지 기다리게
되는데, **그려지지 않은 창은 검은 사각형이고 멈춘 것과 구별되지 않음.** 최적화
빌드에서 1초, 비최적화 빌드에서는 그보다 훨씬 긴 시간임. 지연시키면 창이 즉시
뜨고 결과는 잠시 뒤 로그와 상태 패널(`core self test`)에 나타남.

**T19·T20은 표적 자체를 검사함.** 이전까지 표적은 등속 점산란체 하나뿐이라,
확장표적 처리 기구(클러스터 허용치, 최대셀 거부, 드웰 병합 게이트)가 **1셀 자극으로만**
튜닝된 상태였고 CV 칼만 필터는 틀릴 수 있는 질문을 받아본 적이 없었음.

- **T19** — 150 m 선체는 30 m 분해능에서 여러 셀을 차지해야 함. 산란점마다 자기
  지연과 자기 반송파 위상을 갖게 주입하므로, 거리 확장·종횡비 의존 요동·글린트가
  기하에서 저절로 나옴(따로 붙인 모델이 아님).
- **T20** — 같은 기하에서 직진 168 m vs 6°/s 선회 474 m. **이 2.8배 차이가 IMM이
  메우려는 간격**이고, 만들기 전에 재어 두는 것이 요점임.

**T15–T18은 전파·클러터 물리를 바깥 기준에 맞춰 검사함.**

- **T15** — 25 m 안테나와 5 m 표적의 평활구 수평선은 4.12·(√25+√5) = 29.8 km.
  그 안에서는 R⁻⁴만큼만 떨어지고(측정 4.7 dB, 이론 3.9), 넘어가면 급격히
  꺼져야 함(측정 25.3 dB). 평면지구 모델은 이 구간에서 아무것도 잃지 않음.
- **T16** — csc² 안테나의 정의적 성질은 **고정 고도 표적의 수신전력이 거리에
  무관**하다는 것. 이득이 R²로 자라 양방향 R⁴가 되어 레이다 방정식의 R⁻⁴를
  정확히 상쇄함. 같은 고도 4개 표적의 산포가 채움 켜면 0.27 dB, 끄면 43.4 dB.
- **T17** — 2펄스 캔슬러가 지울 수 있는 것은 펄스간 상관이 유지되는 성분뿐이므로
  개선도는 클러터 스펙트럼 폭이 정함. 그 폭에는 해면 운동뿐 아니라 **빔이 지나가는
  것**(0.265/t_dwell)도 들어감. 회전이 느려지면 그만큼 잘 지워져야 하고, 실측
  +5.9 dB는 이론 1/(1−ρ)가 예측하는 값과 일치함. 이전 모델에서는 회전 속도를
  바꿔도 개선도가 변하지 않았음.
- **T18** — 이 라이브러리의 모든 CFAR 승수는 셀 전력이 지수분포라는 가정에서
  유도됨. 시뮬레이터의 바다도 복소 가우시안이던 동안에는 검출기와 시뮬레이터가
  같은 가정을 공유해 **오경보율 측정이 자기충족**이었음. 복합 K 텍스처를 켜면
  같은 설계 Pfa에서 오경보가 형상모수에 따라 단조 증가함 — 그 차이가 실제
  검출기가 다른 추정기나 다른 임계, 스캔간 적분으로 되사야 하는 마진임.
...
T10은 그보다 훨씬 좁게 0.5배–2배로 조임. 다섯 종류를 **임계표본 형상**에서
돌리기 때문임 — 사각 테이퍼, 도플러 과표본 없음, 표본율 = 대역폭, 거리축 전용
